...
func TestValidator(t *testing.T) {
	...
	// Errs object based on the errors that field validators return.
	...
	t.Run("ValidatorUsername", func(t *testing.T) {
		...
		for _, c := range []struct {
			...
			t.Run(c.name, func(t *testing.T) {
				assert.EqualArr(t, c.wantErrs, sut.Validate(c.username))
			})
		}
	})

	t.Run("ValidatorPassword", func(t *testing.T) {
		...
		for _, c := range []struct {
			...
			t.Run(c.name, func(t *testing.T) {
				assert.EqualArr(t, c.wantErrs, sut.Validate(c.password))
			})
		}
	})
}
